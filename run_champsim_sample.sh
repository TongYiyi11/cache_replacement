...
if [ "$#" -lt 5 ]; then
    ...
    echo "Usage: ./run_champsim.sh [BINARY] [N_WARM] [N_SIM] [TRACE] [SAMPLE_SIZE] [OPTION]"
    exit 1
fi
...
mkdir -p results_${N_SIM}M_sample
(./bin/${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 ${OPTION} -traces ${TRACE_DIR}/${TRACE}) &> results_${N_SIM}M_sample/${TRACE}-${BINARY}${OPTION}-${SAMPLE_SIZE}.txt
